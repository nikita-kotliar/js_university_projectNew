import{h as F}from"./assets/exercises_card-DQEWkzfT.js";import{i as f}from"./assets/vendor-I1I71QQ2.js";const i={filters:document.querySelector(".filters"),navButtons:document.querySelector(".nav-buttons"),musclesBtn:document.querySelector(".muscles-btn"),exercisesTitle:document.querySelector(".exercises-title"),searchForm:document.querySelector(".search-form"),loadMoreBtn:document.querySelector(".load-more-btn"),quoteContainer:document.querySelector(".quote"),pagination:document.querySelector(".pagination"),exercisesWrapper:document.querySelector(".exercises-div"),exercises:document.querySelector(".exercises-list")};let k=window.innerWidth<768?8:12,n=1,g="Muscles",q="Muscles",B="",b="",w="",m=[];O();x();i.musclesBtn.classList.add("active-btn");i.filters.addEventListener("click",I);i.exercises.addEventListener("click",W);i.searchForm.addEventListener("input",D);var M;(M=i.loadMoreBtn)==null||M.addEventListener("click",loadMore);async function x(e=!0){e&&(n=1,i.exercises.innerHTML="");let t=`https://your-energy.b.goit.study/api/filters?filter=${g}&page=${n}&limit=${k}`;q.trim()&&(t+=`&name=${q}`);const a=await(await fetch(t)).json();if(!a.results.length){C();return}j(a.results),H(a.totalPages)}function j(e){const t=`
    <ul class="exercises">
