import{d as N,h as C}from"./assets/exercises_card-nG6lz2Kr.js";import{i as p}from"./assets/vendor-I1I71QQ2.js";const r={filters:document.querySelector(".filters"),navButtons:document.querySelector(".nav-buttons"),musclesBtn:document.querySelector(".muscles-btn"),exercisesTitle:document.querySelector(".exercises-title"),searchForm:document.querySelector(".search-form"),loadMoreBtn:document.querySelector(".load-more-btn"),quoteContainer:document.querySelector(".quote"),pagination:document.querySelector(".pagination"),exercisesWrapper:document.querySelector(".exercises-div"),exercises:document.querySelector(".exercises-list")};let F=window.innerWidth<768?9:12,H=window.innerWidth<768?8:10,a=1,f="Muscles",T="Muscles",k="",x="",w="",m=[];N(r.quoteContainer);b();r.musclesBtn.classList.add("active-btn");r.filters.addEventListener("click",W);r.exercises.addEventListener("click",I);r.searchForm.addEventListener("input",O);var q;(q=r.loadMoreBtn)==null||q.addEventListener("click",loadMore);async function b(e=!0){e&&(a=1,r.exercises.innerHTML="");let t=`https://your-energy.b.goit.study/api/filters?filter=${f}&page=${a}&limit=${F}`;T.trim()&&(t+=`&name=${T}`);const n=await(await fetch(t)).json();if(!n.results.length){B();return}_(n.results),j(n.totalPages)}function _(e){const t=`
    <ul class="exercises">
