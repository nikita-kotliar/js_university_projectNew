import{h as _}from"./assets/exercises_card-BlUP6G50.js";import{i as p}from"./assets/vendor-I1I71QQ2.js";const r={filters:document.querySelector(".filters"),navButtons:document.querySelector(".nav-buttons"),musclesBtn:document.querySelector(".muscles-btn"),exercisesTitle:document.querySelector(".exercises-title"),searchForm:document.querySelector(".search-form"),loadMoreBtn:document.querySelector(".load-more-btn"),quoteContainer:document.querySelector(".quote"),pagination:document.querySelector(".pagination"),exercisesWrapper:document.querySelector(".exercises-div"),exercises:document.querySelector(".exercises-list")};let k=window.innerWidth<768?8:12,a=1,f="Muscles",q="Muscles",j="",b="",w="",m=[];D();x();r.musclesBtn.classList.add("active-btn");r.filters.addEventListener("click",C);r.exercises.addEventListener("click",F);r.searchForm.addEventListener("input",W);var M;(M=r.loadMoreBtn)==null||M.addEventListener("click",loadMore);async function x(e=!0){e&&(a=1,r.exercises.innerHTML="");let t=`https://your-energy.b.goit.study/api/filters?filter=${f}&page=${a}&limit=${k}`;q.trim()&&(t+=`&name=${q}`);const n=await(await fetch(t)).json();if(!n.results.length){B();return}H(n.results),N(n.totalPages)}function H(e){const t=`
    <ul class="exercises">
  ...
  `;r.exercises.insertAdjacentHTML("beforeend",t)}function C(e){var t;e.target.tagName==="BUTTON"&&((t=document.querySelector(".active-btn"))==null||t.classList.remove("active-btn"),e.target.classList.add("active-btn"),e.target.classList.contains("muscles-btn")?f="Muscles":e.target.classList.contains("bodyparts-btn")?f="Body parts":e.target.classList.contains("equipment-btn")&&(f="Equipment"),r.exercisesTitle.textContent="Exercises",r.searchForm.style.display="none",x(!0))}async function F(e){const t=e.target.closest(".exercise");if(!t)return;const s=t.querySelector(".exercise-filter"),n=t.querySelector(".exercise-subtitle");!s||!n||(b=s.textContent,w=n.textContent.toLowerCase(),r.exercisesTitle.innerHTML=`
    <ul class="exercises-title">
      Exercises / <span>${g(w)}</span>
    </ul>
  `,r.searchForm.style.display="block",a=1,r.exercises.innerHTML="",await L())}async function L(e=!0){e&&(m=[]);let t=b.toLowerCase();t==="body parts"&&(t="bodypart");const s=`
  ...
  `.replace(/\s+/g,""),i=await(await fetch(s)).json();if(!i.results.length){B();return}m=i.results,I(i.results),N(i.totalPages)}async function I(e){r.exercises.innerHTML="",m=e;const t=`
    <ul class="exercises-cards">
      ${e.map(({name:s,_id:n,rating:i,burnedCalories:l,bodyPart:o,target:u,time:v})=>{let d=`${l} / ${v} min`,E="...",S=u;return i%1===0&&(i+=".0"),i=parseFloat(i).toFixed(1),s.length>32&&(s=s.slice(0,24)+"..."),window.innerWidth<1440&&(s.length>24&&(s=s.slice(0,20)+"..."),E="...",S="..."),window.innerWidth<768&&(d=`${l} /... min`,s.length>22&&(s=s.slice(0,18)+"...")),`
  ...
  `;r.exercises.insertAdjacentHTML("beforeend",t)}function W(e){j=e.target.value.trim().toLowerCase(),a=1,r.exercises.innerHTML="",L(!0)}function N(e){if(!r.pagination)return;if(e<=1){r.pagination.innerHTML="";return}let t="";const s=(o,u,v,d)=>`
    <li>
      <button
        name="Next page"
  ...
  `;t+=s("left","icon-big",a-2,a<=2),t+=s("left","icon-small",a-1,a===1);let i=Math.max(1,a-1),l=Math.min(e,a+1);a===1&&(l=Math.min(e,3)),a===e&&(i=Math.max(1,e-2)),i>1&&(t+=n(1),i>2&&e>4&&(t+='<li class="dots">...</li>'));for(let o=i;o<=l;o++)t+=n(o);l<e&&(l<e-1&&e>4&&(t+='<li class="dots">...</li>'),t+=n(e)),t+=s("right","icon-small",a+1,a===e),t+=s("right","icon-big",a+2,a>=e-1),r.pagination.innerHTML=t}r.pagination.addEventListener("click",e=>{const t=e.target.closest(".pagination-btn");if(!t)return;const s=Number(t.dataset.page);s!==a&&(a=s,r.exercises.innerHTML="",b?L(!1):x(!1))});r.exercises.addEventListener("click",e=>{const t=e.target.closest('[data-action="start"]');if(!t)return;const s=m.find(n=>n._id===t.dataset.id);_(s)});function B(){var e;r.exercises.innerHTML=`
